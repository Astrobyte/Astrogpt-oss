    ...
    uint num_simdgroups [[simdgroups_per_threadgroup]])
{
    ...
    if (cumsum >= sample_cdf) {
        block_idx = metal::min(tid, block_idx);
        block_sum = 0.0f;
    }
    ...
    block_idx = metal::simd_min(block_idx);
    block_sum = metal::simd_max(block_sum);
    block_idx = metal::min(block_idx, args.num_blocks - 1);

    uint offset = args.num_dims_per_block * block_idx + tid;
    float accumulated_sum = block_sum;
    uint sample_idx;
    do {
        // Find the token: the smallest tid where sample_cdf >= s
        float cumsum = 0.0f;
        if (offset < args.num_dims) {
            cumsum = prob[offset];
        }
        cumsum = metal::simd_prefix_inclusive_sum(cumsum);
        if (simdgroup_tid == 31) {
        ...
        sample_idx = args.num_dims;
        if (cumsum >= sample_cdf) {
            sample_idx = metal::min(offset, sample_idx);
        }
    ...
    } while (sample_idx == args.num_dims);
